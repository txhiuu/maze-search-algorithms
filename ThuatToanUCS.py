...
def KhoiPhuc(came_from, start, goal):
    path_nodes = []
    current = goal
    while current != start:
    ...
    while open_heap:
        ...
        for neighbor in graph[current]:
            ...
            if neighbor not in g_score or tentative_g < g_score[neighbor]:
                ...
                print(f"    Thêm ô {neighbor} vào heap, g={tentative_g}")

    return None


def main():
    print("BẮT ĐẦU CHƯƠNG TRÌNH GIẢI MÊ CUNG BẰNG UCS")
    ...
    solution = UCS(start_pos, end_pos)
    elapsed_time = time.time() - start_time
    print(f"\nThời gian chạy UCS: {elapsed_time:.4f} giây")
